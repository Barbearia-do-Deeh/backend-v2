...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'POST') {
      ...
      // data_pagamento é opcional — quando não vem, usa hoje (comportamento de
      // sempre). Quando vem (ex: registrando um pagamento atrasado ou de uma
      // data passada), usa a data escolhida tanto pro registro quanto pra
      // calcular o fim do ciclo (30 dias a partir DAQUELA data, não de hoje).
      ...
      const result = await client.query(
        ...
        [cliente_id, plano, valor, forma_pagamento || 'pix', dataRef, fim]
      );
      return res.status(200).json({ success: true, id: result.rows[0].id, valor, data_pagamento: dataRef, ciclo_fim: fim });
    }
    if (req.method === 'GET') {
