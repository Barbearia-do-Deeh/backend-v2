const VALOR_PLANO = {
  essencial: 160,
  classico: 260,
  empresario: 340,
};
function periodoParaDatas(mes, ano) {
  const inicio = `${ano}-${String(mes).padStart(2, '0')}-01`;
  const fimDate = new Date(Date.UTC(ano, mes, 0));
  const fim = fimDate.toISOString().slice(0, 10);
  return { inicio, fim };
}

// ---- Núcleo do cálculo — aceita qualquer intervalo de datas (semana, quinzena,
// mês, ano, personalizado). Tudo que antes só existia por mês/ano agora usa isso
// por baixo — calcularResumoFinanceiro (mês/ano) é só um atalho que já resolve
// as datas e chama esta função.
async function calcularResumoPeriodo(client, { inicio, fim }) {
  const atendimentosResult = await client.query(
    `SELECT forma_pagamento, valor_cobrado, valor_produtos, servicos FROM atendimentos
     WHERE data_hora::date BETWEEN $1 AND $2`,
    [inicio, fim]
  );
  const pagamentosResult = await client.query(
    `SELECT valor, plano FROM pagamentos_planos
     WHERE data_pagamento BETWEEN $1 AND $2`,
    [inicio, fim]
  );
  const despesasResult = await client.query(
    `SELECT categoria, valor FROM despesas
     WHERE data BETWEEN $1 AND $2`,
    [inicio, fim]
  );
  // Traz plano + valor_pacote de cada cliente ativo, pra usar o valor
  // personalizado (quando definido) em vez do valor fixo do plano.
  // OBS: isso é uma "foto" de agora (clientes com plano ativo hoje), não do
  // período — MRR estimado é sempre o MRR atual, não muda com o período escolhido.
  const clientesAtivosResult = await client.query(
    `SELECT plano, valor_pacote FROM clientes WHERE plano != 'nenhum'`
  );
  let receitaServicosAvulsos = 0;
  let receitaProdutos = 0;
  let qtdAtendimentos = atendimentosResult.rows.length;
  let qtdAvulsos = 0;
  let qtdCobertosPacote = 0;
  for (const row of atendimentosResult.rows) {
    receitaServicosAvulsos += parseFloat(row.valor_cobrado) || 0;
    receitaProdutos += parseFloat(row.valor_produtos) || 0;
    if (row.forma_pagamento === 'avulso' || row.forma_pagamento === 'misto') qtdAvulsos++;
    if (row.forma_pagamento === 'pacote' || row.forma_pagamento === 'misto') qtdCobertosPacote++;
  }
  let receitaPlanos = 0;
  const receitaPlanosPorTipo = { essencial: 0, classico: 0, empresario: 0 };
  for (const row of pagamentosResult.rows) {
    const valor = parseFloat(row.valor) || 0;
    receitaPlanos += valor;
    if (receitaPlanosPorTipo[row.plano] !== undefined) receitaPlanosPorTipo[row.plano] += valor;
  }
  let totalDespesas = 0;
  const despesasPorCategoria = {};
  for (const row of despesasResult.rows) {
    const valor = parseFloat(row.valor) || 0;
    totalDespesas += valor;
    despesasPorCategoria[row.categoria] = (despesasPorCategoria[row.categoria] || 0) + valor;
  }
  const clientesAtivosPorPlano = {};
  let mrrEstimado = 0;
  for (const row of clientesAtivosResult.rows) {
    clientesAtivosPorPlano[row.plano] = (clientesAtivosPorPlano[row.plano] || 0) + 1;
    const valorCliente = row.valor_pacote !== null && row.valor_pacote !== undefined
      ? parseFloat(row.valor_pacote)
      : (VALOR_PLANO[row.plano] || 0);
    mrrEstimado += valorCliente;
  }
  const receitaTotal = receitaServicosAvulsos + receitaProdutos + receitaPlanos;
  const lucroLiquido = receitaTotal - totalDespesas;
  const ticketMedio = qtdAtendimentos > 0 ? receitaTotal / qtdAtendimentos : 0;
  return {
    periodo: { inicio, fim },
    receita: {
      servicos_avulsos: Number(receitaServicosAvulsos.toFixed(2)),
      produtos: Number(receitaProdutos.toFixed(2)),
      planos: Number(receitaPlanos.toFixed(2)),
      planos_por_tipo: receitaPlanosPorTipo,
      total: Number(receitaTotal.toFixed(2)),
    },
    despesas: {
      total: Number(totalDespesas.toFixed(2)),
      por_categoria: despesasPorCategoria,
    },
    lucro_liquido: Number(lucroLiquido.toFixed(2)),
    atendimentos: {
      quantidade: qtdAtendimentos,
      avulsos: qtdAvulsos,
      cobertos_pelo_pacote: qtdCobertosPacote,
      ticket_medio: Number(ticketMedio.toFixed(2)),
    },
    clientes_ativos_por_plano: clientesAtivosPorPlano,
    mrr_estimado: Number(mrrEstimado.toFixed(2)),
  };
}

// Atalho por mês/ano — mantido pra quem ainda chamar assim; por baixo já usa
// calcularResumoPeriodo. periodo de retorno inclui mes/ano além de inicio/fim.
async function calcularResumoFinanceiro(client, { mes, ano }) {
  const { inicio, fim } = periodoParaDatas(mes, ano);
  const resumo = await calcularResumoPeriodo(client, { inicio, fim });
  return { ...resumo, periodo: { mes, ano, inicio, fim } };
}

// ---- Retenção de clientes ----
// Pra cada cliente cadastrado, olha o atendimento mais recente (tabela `atendimentos`,
// casado por cliente_id) e classifica em faixas de dias desde a última visita.
// "Nunca visitou" cobre cliente cadastrado (ex: só salvou o Perfil) que nunca teve
// atendimento registrado. Isso é sempre uma "foto" de agora — não depende de período.
async function calcularRetencao(client) {
  const result = await client.query(
    `SELECT c.telefone, MAX(a.data_hora) AS ultima_visita
     FROM clientes c
     LEFT JOIN atendimentos a ON a.cliente_id = c.id
     GROUP BY c.telefone`
  );

  const hoje = new Date();
  let ativos30 = 0;
  let inativos3160 = 0;
  let inativos6190 = 0;
  let inativos90mais = 0;
  let nuncaVisitou = 0;

  for (const row of result.rows) {
    if (!row.ultima_visita) { nuncaVisitou++; continue; }
    const dias = Math.floor((hoje - new Date(row.ultima_visita)) / (1000 * 60 * 60 * 24));
    if (dias <= 30) ativos30++;
    else if (dias <= 60) inativos3160++;
    else if (dias <= 90) inativos6190++;
    else inativos90mais++;
  }

  return {
    total_clientes: result.rows.length,
    ativos_30: ativos30,
    inativos_31_60: inativos3160,
    inativos_61_90: inativos6190,
    inativos_90_mais: inativos90mais,
    nunca_visitou: nuncaVisitou,
  };
}

module.exports = {
  calcularResumoFinanceiro, calcularResumoPeriodo, VALOR_PLANO, periodoParaDatas, calcularRetencao,
};
