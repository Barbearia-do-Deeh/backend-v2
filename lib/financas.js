...
async function calcularResumoFinanceiro(client, { mes, ano }) {
  ...
  const clientesAtivosResult = await client.query(
    `SELECT plano, COUNT(*) as total FROM clientes
     WHERE plano != 'nenhum' GROUP BY plano`
  );
  ...
  for (const row of clientesAtivosResult.rows) {
    clientesAtivosPorPlano[row.plano] = parseInt(row.total, 10);
    mrrEstimado += (VALOR_PLANO[row.plano] || 0) * parseInt(row.total, 10);
  }
